...
function openURLInCurrentTab(search: any) {
  ...
  //chrome.runtime.sendMessage({ action: 'OpenURLInCurrentTab', url });
}
...
function openURLInNewTab(url: any) {
  // Send a message to the background script to request opening a URL in the next tab
  chrome.runtime.sendMessage({ action: 'openURLInNextTab', url});
}
...
if (request.action === 'clickRandomAds1') {
  ...
  if (request.action === 'openUrlNewTab') {
    const urlToOpen = request.data.selector;
